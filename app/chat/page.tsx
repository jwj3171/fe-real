...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    s.on("chat:system", (m) => setMsgs((prev) => [...prev, { ...m, system: true }]));
    s.on("chat:message", (m) => setMsgs((prev) => [...prev, m]));
    s.on("connect_error", (err) =>
      setMsgs((prev) => [
        ...prev,
        { text: `연결 오류: ${err?.message ?? "unknown"}`, ts: Date.now(), system: true },
      ])
    );
  ...
  return (
    <div className="mx-auto max-w-2xl p-4">
      <h1 className="text-2xl font-semibold mb-4">공개 채팅</h1>
      ...
      {!joined ? (
        ...
        <>
          <div
            ref={listRef}
            className="h-[60vh] w-full overflow-y-auto rounded border p-3 space-y-2 bg-white"
          >
            {msgs.map((m: any, idx: number) =>
              m.system ? (
                <div key={`s-${idx}`} className="text-center text-xs text-gray-500">
                  {new Date(m.ts).toLocaleTimeString()} · {m.text}
                </div>
              ) : (
                <div key={`${m.id}-${m.ts}`} className="text-sm">
                  <span className="font-semibold mr-2">{m.nickname}</span>
          ...
          >
            <input
              ...
              maxLength={500}
            />
            <button className="rounded bg-black px-4 py-2 text-white">보내기</button>
          </form>
        </>
      )}
    </div>
